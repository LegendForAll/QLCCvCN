...
Public Class FrmRULE
    ...
    Private Sub FrmRULE_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        'load qui so cay toi da
        Dim command As New SqlCommand("SELECT * FROM [QUIDINH]", connection)
        Dim adapter As New SqlDataAdapter(command)
        Dim table As New DataTable()
        adapter.Fill(table)
        cbx_CayMax.DataSource = table
    ...
    Private Sub btn_Up_CayMax_Click(sender As Object, e As EventArgs) Handles btn_Up_CayMax.Click
        If (tbx_CayMax.Text = "") Then
            MessageBox.Show("Number tree is not NULL.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        Else
            Dim updateQuery As String = "UPDATE [QUIDINH] SET [SL_MAX] = '" & tbx_CayMax.Text & "'
                                                            WHERE [ID_QD] = '" & cbx_QD_Tien.SelectedValue & "'"
            ExcuteQuery(updateQuery)
            MessageBox.Show("Update tree number max successful.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End If
    End Sub
End Class